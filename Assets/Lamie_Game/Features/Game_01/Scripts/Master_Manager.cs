...
public class MasterManager : MonoBehaviour
{
    public static readonly Dictionary<string, string[]> ItemNames = new Dictionary<string, string[]>
    {
        ...
        { "اكتشاف الاختلافات البصرية", new[] {
            "يكتشف العنصر المختلف بوضوح ضمن مجموعة عناصر متشابهة",
            "يميز الفروق البصرية البسيطة بين العناصر دون خلط",
            "يحدد العنصر المختلف مع تجاهل المشتتات غير المرتبطة",
            "يكتشف الفروق الدقيقة بين عناصر متشابهة بدرجة عالية",
            "يحافظ على دقة الأداء مع زيادة عدد العناصر المعروضة",
            "يُظهر نمط أداء منظما يقل فيه الاعتماد على المحاولة العشوائية"
        }},
    ...
    private IEnumerator AutoRestartTimer(float delay)
    {
    ...
    public void StartBackendTracking(string gameName)
    {
        ...
        if (gameName == "مطابقة العناصر البصرية وفق اللون")
        {
            StartCoroutine(AutoRestartTimer(3.0f));
        }
        else if (gameName == "مطابقة العناصر البصرية وفق الشكل")
        {
            StartCoroutine(AutoRestartTimer(0.5f));
        }
    }
    ...
    public void SubmitStageData()
    {
        ...
        if (currentIndicatorName == "مطابقة العناصر البصرية وفق اللون" || currentIndicatorName == "مطابقة العناصر البصرية وفق الشكل")
        {
            StartCoroutine(AutoRestartTimer(2.5f));
        }
    }
    ...
    IEnumerator GoToGame3()
    {
        ...
        StartBackendTracking("اكتشاف الاختلافات البصرية");
